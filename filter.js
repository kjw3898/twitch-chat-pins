...
function setFilterList() {
  chrome.runtime.sendMessage(
    { method: "setFilterList", filterList: filterPinedUserList },
    function (response) {
      console.log(response.data);
    }
  );
...
function addPinButton(node) {
  temp_nickname = node
    .querySelector("figure[aria-label]")
    .getAttribute("aria-label");
  let wisperButton = node.querySelector(
    'button[data-a-target="usercard-whisper-button"]'
  );
  if (wisperButton) {
    ...
    let pinButton = pinButtonDiv.querySelector(
    ...
    if (filterPinedUserList.indexOf(temp_nickname) != -1) {
      ...
      ).innerText = chrome.i18n.getMessage("unpinChatting");
    } else {
      pinButton.querySelector(
        '[data-a-target="tw-core-button-label-text"]'
      ).innerText = chrome.i18n.getMessage("pinChatting");
    }
  ...
  mutations.forEach(function (mutation) {
    mutation.addedNodes.forEach(function (node) {
      if (node.tagName == "LI") {
        ...
        if (
          filterPinedUserList.indexOf(username) != -1 ||
          filterPinedUserList.indexOf(nickname) != -1
        ) {
...
function lessButtonCilck() {
  //lessButton Click Event Handler
  lessMode = !lessMode;
  resizePinnedChatList();
}
...
function initChatPins() {
  ...
  waitForElement(".video-chat__message-list-wrapper").then((selector) => {
    ...
    pinedVODChatListParent.classList.replace(
      "video-chat__message-list-wrapper",
      "tw-border-b"
    );
    parentChatList.insertBefore(
      pinedVODChatListParent,
      parentChatList.lastChild
    );
  ...
  waitForElement(".chat-scrollable-area__message-container").then(
    (selector) => {
      //prevent duplicate
      if (
        document.querySelectorAll(".chat-scrollable-area__message-container")
          .length > 1
      ) {
        return;
      }
      ...
      scrollableChatList =
        selector.parentElement.parentElement.parentElement.parentElement.cloneNode(
          true
        );
      ...
      scrollableChatList.style.borderBottom =
        "var(--border-width-default) solid var(--color-border-base)";
      pinnedLiveChatScrollView = scrollableChatList.querySelector(
        ".simplebar-scroll-content"
      );
      pinedLiveChatList = scrollableChatList.querySelector(
        ".chat-scrollable-area__message-container"
      );
      pinedLiveChatList.className = pinedLiveChatList.className.replace(
        "tw-pd-b-1",
        ""
      );
      ...
      parentChatList.insertBefore(
        scrollableChatList,
        parentChatList.firstChild
      );
      var moreButtonDiv = document.createElement("div"); //add less more control button
      moreButtonDiv.innerHTML =
        '<div class="tw-flex tw-full-width tw-justify-content-center" style="padding: .25rem"><div style="height: 0.5rem; width: 4rem; background-color: var(--color-background-alt-2) !important; border-radius: 0.6rem !important;"></div></div>';
